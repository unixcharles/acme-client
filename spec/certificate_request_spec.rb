...
describe Acme::Client::CertificateRequest do
  ...
  it "checks for a valid digest" do
    expect {
      Acme::Client::CertificateRequest.new(common_name: "example.org", private_key: test_key, digest: :not_a_digest)
  ...
  it "normalizes the subject to OpenSSL short names" do
    subject = Acme::Client::CertificateRequest::SUBJECT_KEYS.each_with_object({}) {|(key, _), subject|
      subject[key] = "example"
    }
    request = Acme::Client::CertificateRequest.new(private_key: test_key, subject: subject)

    subject = Acme::Client::CertificateRequest::SUBJECT_KEYS.each_with_object({}) {|(_, short_name), subject|
  ...
  it "doesn't modify the given names" do
    ...
    original = names.dup
    request = Acme::Client::CertificateRequest.new(common_name: "example.org", names: names, private_key: test_key)
